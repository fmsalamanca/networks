...
# Load data
positions = np.loadtxt('./output/step_00{}/positions.txt'.format(step))
remaining = np.loadtxt('./output/step_00{}/connections.txt'.format(step), dtype=int)
box_curr = np.loadtxt('./output/step_00{}/box.txt'.format(step))
...
broken = np.loadtxt('./output/step_00{}/broken_connections.txt'.format(step), dtype=int)
...
for row in broken:
    i, j = row[0], row[1]
    if i != j:  # valid connection and not self
